 ...
 * Keys are normalized on the way in, because lookups go through `normalizeHebrew`,
 * which folds final letters: "גרם" is only ever found as "גרמ".
 */
const UNIT_ALIASES: ReadonlyMap<string, string> = new Map(
  ...
  ).map(([alias, unit]) => [normalizeHebrew(alias), unit]),
);
