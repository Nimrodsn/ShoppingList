...
בפעם הראשונה אין צורך בכלום מלבד דפדפן: פתיחת `/` על מסד ריק מציגה «איך נקרא
לבית?», ולחיצה על «יצירת הבית» יוצרת את משק הבית, מחברת את המכשיר הזה ומעבירה
ל-«איך קוראים לך?». הטופס נעלם לכולם ברגע שנוצר הבית הראשון, ומאותה נקודה
הכניסה היחידה היא הקישור המשפחתי, שמשתפים ממסך ההגדרות (כולל קוד QR).

ליצירת בית נוסף, או לעבודה בלי דפדפן:
...
4. אחרי הדיפלוי הראשון: פתיחת הדומיין ולחיצה על «יצירת הבית». לחלופין
   `APP_URL=https://<domain> pnpm create-household`.
